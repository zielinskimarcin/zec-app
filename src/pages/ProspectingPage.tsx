...
function orderLeadsForDisplay(leads: ProspectLead[]) {
  const showcase = leads.filter(isShowcaseLead);
  const regular = leads.filter(lead => !isShowcaseLead(lead));
  return [
    ...showcase.slice(0, SHOWCASE_LIMIT),
    ...regular,
    ...showcase.slice(SHOWCASE_LIMIT),
  ];
}
...
export function ProspectingPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPreview() {
      ...
      const preview = orderLeadsForDisplay(((data || []) as GlobalLeadRow[])
        .map(mapLead)
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const matches = orderLeadsForDisplay(rows.map(mapLead)).slice(0, common.maxLeads);
