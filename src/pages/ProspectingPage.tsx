...
import {
  ...
  Briefcase, BarChart2, ExternalLink, SlidersHorizontal
} from 'lucide-react';
...
const PLATFORMS: { id: Platform; label: string; tokenCost: number; icon: React.ReactNode; activeClass: string }[] = [
...
export function ProspectingPage() {
  ...
  // Toggle platform — Google zawsze musi być
  const togglePlatform = (p: Platform) => {
    if (p === 'google') return; // Google zawsze aktywny
    ...
    try {
      ...
      const response = await fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          ...
          package: getPackage(selectedPlatforms), // ← podstawa całego routingu w n8n
